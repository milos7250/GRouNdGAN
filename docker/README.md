# GRouNdScale Docker Image

GRouNdScale simulates single-cell RNA-seq data with gene regulatory network
(GRN)-guided causal generative adversarial networks. This image provides a
ready-to-run GRouNdScale environment with PyTorch, CUDA, and the project
dependencies installed.

- **Docker Hub:** [milos7250/groundscale](https://hub.docker.com/r/milos7250/groundscale)
- **Documentation:** [GRouNdScale documentation](https://milos7250.github.io/GRouNdScale/)
- **Source code:** [github.com/milos7250/GRouNdScale](https://github.com/milos7250/GRouNdScale)
...
- Python 3.12
- GRouNdScale CLI entrypoint: `/groundscale/src/main.py`
...
```bash
docker run --rm milos7250/groundscale:latest
```

## Run GRouNdScale

GRouNdScale requires a configuration file. The simplest approach is to run from
a project directory containing `configs/`, `data/`, and `results/`:

```bash
docker run --rm --gpus all \
  -v "$PWD:/data" \
  milos7250/groundscale:latest \
...
The image has a GRouNdScale entrypoint. Override it when an interactive shell is
needed:
...
Clone the repository and build the `groundscale` image from the repository root:
...
docker run --rm --gpus all \
  -v "$PWD:/data" \
  groundscale --config /data/configs/causal_gan.cfg --help
```
...
the [documentation](https://milos7250.github.io/GRouNdScale/tutorial.html)
...
If you use GRouNdScale, please cite:
...
GRouNdScale is distributed under the [GNU Affero General Public License v3](https://www.gnu.org/licenses/agpl-3.0.en.html).

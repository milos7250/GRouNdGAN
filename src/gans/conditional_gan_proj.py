...
class ConditionalProjGAN(ConditionalGAN):
    ...
    def _get_gradient(
        ...
        **kwargs
        ...
        critic_interpolates = self.crit(interpolates, labels)
    ...
    def _train_critic(self, real_cells, real_labels, c_lambda) -> typing.Tuple[torch.Tensor, torch.Tensor]:
        ...
        fake = self.gen(fake_noise, real_labels)

        crit_fake_pred = self.crit(fake.detach(), real_labels)
        crit_real_pred = self.crit(real_cells, real_labels)

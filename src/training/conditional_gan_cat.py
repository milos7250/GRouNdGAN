...
class ConditionalCatGANTrainer(ConditionalGANTrainer):
    def __init__(
        self,
    ...
    def _generator_step(self) -> tuple["Tensor", "Tensor", "Tensor"]:
        ...
        fake = self.gan.gen(self.gan.cat_one_hot_labels(fake_noise, fake_labels))
        crit_fake_pred = self.gan.crit(self.gan.cat_one_hot_labels(fake, fake_labels))
    ...
    def _critic_step(self, real_cells: "Tensor", real_labels: "Tensor") -> tuple["Tensor", "Tensor", "Tensor"]:
        with torch.no_grad():
            ...
            fake_cells = self.gan.gen(self.gan.cat_one_hot_labels(fake_noise, real_labels))

        crit_fake_pred = self.gan.crit(self.gan.cat_one_hot_labels(fake_cells, real_labels))
        crit_real_pred = self.gan.crit(self.gan.cat_one_hot_labels(real_cells, real_labels))
